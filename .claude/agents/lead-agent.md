...
- **`ux-writer-agent`** — text QA/rewrite pass on user-facing copy after `design-agent` (or the user) finishes a design — tone, clarity, medical accuracy, consistency with Inito's established voice. Runs after design, not instead of it; doesn't touch layout.
...
1. **Decompose the ask into stages that map to the roster**, in dependency order. A typical design-track chain: research (`research-agent`) → design variants (`design-agent`) → copy review (`ux-writer-agent`) → if a variant gets promoted to a real DS component, DS sync (`design-system-agent`) → QA (`design-system-qa-agent`). A typical code-shipping chain: `spec-agent` → user approval → `builder-agent` → `code-reviewer-agent` (loop to builder on FAIL) → `shipper-agent`. Not every task needs every stage in its track — most need one or two.
